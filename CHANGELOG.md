...
- **`pflip_plus_model.py`** — `_compute_scoring_cutoff()`: the `.astype(pd.Int64Dtype())` calls on lines 335/342 converted the entire DataFrame (including `noteIdKey`) from `int64` to `Int64`, creating a type mismatch when merging with `scoringCutoff` (which has `noteIdKey` as `int64`). Fixed by applying `Int64` only to the timestamp value columns (`maxRatingMts`, `nthRatingMts`), leaving `noteIdKey` as `int64`. Also cast `notes[createdAtMillisKey]` to `Int64` alongside the existing `noteStatusHistory[createdAtMillisKey]` cast to keep merge keys consistent.
